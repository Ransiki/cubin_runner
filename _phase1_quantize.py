...
def quant_fp4(a):
    ...
    a_fp4, a_sf = torch.ops.trtllm.fp4_quantize(a.cuda(), global_sf.cuda(), 16, False, False)
...
def main():
    ...
    g1_fp4 = torch.stack(g1_fp4_all).view(torch.float8_e4m3fn).reshape(E, I2, H//2)
    g1_sf = torch.stack(g1_sf_all).view(torch.float8_e4m3fn).reshape(E, I2, H//16)
    g2_fp4 = torch.stack(g2_fp4_all).view(torch.float8_e4m3fn).reshape(E, H, I//2)
    g2_sf = torch.stack(g2_sf_all).view(torch.float8_e4m3fn).reshape(E, H, I//16)
    ...
    g1s_w, g1s_sf = [], []
    for i in range(E):
        wr = reorder_rows_for_gated_act_gemm(g1_fp4[i].clone())
        sr = reorder_rows_for_gated_act_gemm(g1_sf[i].clone())
    ...
    g1s_sf = torch.stack(g1s_sf).view(torch.float8_e4m3fn).reshape(E, I2, H//16)
    ...
    g2s_sf = torch.stack(g2s_sf).view(torch.float8_e4m3fn).reshape(E, H, I//16)
    ...
    def dequant_fp4(fp4_bytes, sf_bytes, inv_gsf, sf_vec=16):
        return torch.ops.tensorrt_llm.e2m1_and_ufp8sf_scale_to_float_v2(
            fp4_bytes.cpu(), sf_bytes.cpu().reshape(-1),
            torch.tensor(inv_gsf).float().cpu(), sf_vec, 1, True)

    fc1_deq_list, fc2_deq_list = [], []
    for i in range(E):
        fc1_deq_list.append(dequant_fp4(g1_fp4_all[i], g1_sf_all[i], 1.0 / g1_gsf[i].item()).cuda())
        fc2_deq_list.append(dequant_fp4(g2_fp4_all[i], g2_sf_all[i], 1.0 / g2_gsf[i].item()).cuda())
