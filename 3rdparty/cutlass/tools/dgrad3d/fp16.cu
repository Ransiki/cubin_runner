...
void Dgrad_3d_ndhwc_cpu(KernelParams<ElementType> params){
    ...
    for(int i = 0;i< N*C*D*H*W;++i){
        params.gmem_image[i] = __float2half(__half2float(params.gmem_image[i])*beta);
    }
                                    ...
                                    for(int s=0;s<S;++s){
                                        ...
                                        if((unsigned)d<D && (unsigned)h<H && (unsigned)w<W){
                                            image_index = n * imageStrideN
                                                ...
                                                + h * imageStrideH
                                                + w * imageStrideW;
                                            params.gmem_image[image_index] = __float2half(__half2float(params.gmem_image[image_index])
                                                    +__half2float(params.gmem_error[error_index])
                                                    *__half2float(params.gmem_filter[filter_index])*alpha);
                                        }
                                    }
                                }
                            }
...
static_assert(T_PER_CTA == 2, "T_PER_CTA == 2");
static_assert(R_PER_CTA == 2, "R_PER_CTA == 2");
static_assert(S_PER_CTA == 2, "S_PER_CTA == 2");
...
#pragma unroll
    ...
    __shared__ __half2 smem_error[O_PER_CTA*P_PER_CTA*Q_PER_CTA*SMEM_ERROR_K];
    __half2 *smem_error_sts_ptr = smem_error + (tidx
            + smem_q_begin*SMEM_ERROR_K
            + smem_p_begin*SMEM_ERROR_K*Q_PER_CTA
            + smem_o_begin*SMEM_ERROR_K*Q_PER_CTA*P_PER_CTA);
    __half2 *smem_error_lds_ptr_base = smem_error_sts_ptr;
    __half2 *smem_error_lds_ptr = smem_error_lds_ptr_base;
#pragma unroll
    for(int o_index = 0;o_index<O_PER_THREAD;++o_index){
#pragma unroll
        for(int p_index = 0;p_index<P_PER_THREAD;++p_index){
#pragma unroll
            for(int q_index = 0;q_index<Q_PER_THREAD;++q_index){
                smem_error_sts_ptr[q_index*SMEM_ERROR_K
                    ...
                    +o_index*SMEM_ERROR_K*Q_PER_CTA*P_PER_CTA] = __half2half2(error_reg[o_index][p_index][q_index]);
            }
        }
    }
    __shared__ __half2 smem_filter[C_PER_CTA*T_PER_CTA*R_PER_CTA*K_PER_CTA_PER_LOOP];
    __half2 *smem_filter_sts_ptr = smem_filter + (tidx
            + tidy*C_PER_CTA * R_PER_CTA * T_PER_CTA);
    __half2 *smem_filter_lds_ptr_base = smem_filter + tidx+ lane_id*C_PER_CTA*T_PER_CTA*R_PER_CTA;
    __half2 *smem_filter_lds_ptr = smem_filter_lds_ptr_base;
#pragma unroll
    for(int t_index=0;t_index<T;++t_index){
#pragma unroll
        for(int r_index=0;r_index<R;++r_index){
            if(IS_CROSS_CORRELATION){
                smem_filter_sts_ptr[r_index*C_PER_CTA+t_index*C_PER_CTA*R_PER_CTA] = 
                    __halves2half2(filter_reg[t_index][r_index][0],filter_reg[t_index][r_index][1]);
            }
            else{
                smem_filter_sts_ptr[r_index*C_PER_CTA+t_index*C_PER_CTA*R_PER_CTA] = 
                    __halves2half2(filter_reg[t_index][r_index][1],filter_reg[t_index][r_index][0]);
            }
        }
    }
    //Main loop
    ...
    //Allocate math reg
    __half2 error_math[O_PER_THREAD][P_PER_THREAD][Q_PER_THREAD];
    __half2 filter_math[T][R];
    __half2 image_math[D_PER_THREAD][H_PER_THREAD][W_PER_THREAD/2];
    Clear<__half2,D_PER_THREAD,H_PER_THREAD,W_PER_THREAD/2>(image_math);
    __syncthreads();
...
#pragma unroll
                for(int r_index = 0;r_index<R;++r_index){
                    filter_math[t_index][r_index] = smem_filter_lds_ptr[r_index * C_PER_CTA + t_index * C_PER_CTA * R_PER_CTA];
                }
            }
            //Math
#pragma unroll
...
#pragma unroll
                            for(int r_index = 0;r_index<R;++r_index){
                                image_math[o_index*STRIDE_D+t_index*DILATION_D][p_index*STRIDE_H+r_index*DILATION_H][q_index] =
                                    __hfma2(error_math[o_index][p_index][q_index],
                                            filter_math[(IS_CROSS_CORRELATION?t_index:(T-1-t_index))]
                                            [(IS_CROSS_CORRELATION?r_index:(R-1-r_index))],
                                            image_math[o_index*STRIDE_D+t_index*DILATION_D]
                                            [p_index*STRIDE_H+r_index*DILATION_H]
                                            [q_index]);
            ...
            smem_filter_lds_ptr = smem_filter_lds_ptr_base + (k_index + 1) * C_PER_CTA*R_PER_CTA*T_PER_CTA;
            if(lane_id +k_index + 1>=K_PER_CTA_PER_LOOP){
                smem_filter_lds_ptr -= K_PER_CTA_PER_LOOP*C_PER_CTA*R_PER_CTA*T_PER_CTA;
            }
        }
        __syncthreads();
        //Update ptr
...
#pragma unroll
    ...
    ElementType image_result[D_PER_THREAD][H_PER_THREAD][W_PER_THREAD];
#pragma unroll
    for(int d_index=0;d_index<D_PER_THREAD;++d_index){
#pragma unroll
        for(int h_index=0;h_index<H_PER_THREAD;++h_index){
#pragma unroll
            for(int w_index=0;w_index<W_PER_THREAD/2;++w_index){
                image_result[d_index][h_index][w_index*2] = __float2half(__half2float(__low2half(image_math[d_index][h_index][w_index]))
                        *params.alpha); 
                image_result[d_index][h_index][w_index*2+1] = __float2half(__half2float(__high2half(image_math[d_index][h_index][w_index]))
                        *params.alpha); 
            }
        }
    }
...
#pragma unroll
            for(int w_index=0;w_index<W_PER_THREAD;++w_index){
                if(c_valid && unsigned(thread_d_begin+d_index)<D
                    ...
                    *(ElementType*)((std::uint64_t)image_ptr+OFFSET_LUT[OPQ_PER_THREAD+K_PER_CTA_PER_LOOP+w_index+h_index*W_PER_THREAD+d_index*W_PER_THREAD*H_PER_THREAD]) = image_result[d_index][h_index][w_index];
                }
            }
        }
    }
...
#pragma unroll
    ...
    double hfma_sol_cycle = 1.0*params.N*params.C*params.K*params.T*params.R*params.S*params.O*params.P*params.Q/80/128;
        ...
        bool crc_check){
            ...
            if(crc_check){
                ...
                for(int i = 0;i<NCDHW;++i){
                    if(fabs(__half2float(gmem_image_h_ref[i])-__half2float(gmem_image_h[i]))>0.1){
                        error_position = i;
                        break;
                    }
                }
                ...
                if(error_position==-1){
                    ...
                    std::cout << "n=" << error_n << " "
                        ...
                        << "h=" << error_h << " "
                        << "w=" << error_w << " " << std::endl;
                    std::cout << "ref = " << __half2float(gmem_image_h_ref[element_position]) << std::endl;
                    std::cout << "val = " << __half2float(gmem_image_h[element_position]) << std::endl;
                }
